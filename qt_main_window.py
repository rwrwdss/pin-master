...
# Патч для работы webdriver-manager в frozen режиме (должен быть импортирован до webdriver_manager)
try:
    import webdriver_frozen_patch  # noqa: F401
except ImportError as e:
    logging.getLogger('PinMaster').warning("webdriver_frozen_patch не найден: %s", e)
...
try:
    from board_scraper import get_boards as board_scraper_get_boards
...
# В frozen-режиме лог в ~/Library/Application Support/PinMaster/logs, иначе рядом со скриптом
if is_frozen():
    log_file = str(get_logs_dir() / 'pinmaster.log')
else:
    log_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'pinmaster.log')
...
if __name__ == "__main__":
    ...
    try:
        ...
        app.setStyle("Fusion")  # QSS корректно работает только со стилем Fusion (не native macOS)
        ...
        # Повторное применение QSS после показа окна (на macOS стили лучше применяются к уже отрисованному дереву)
        QTimer.singleShot(100, window.load_styles)
        # Фиксируем размер окна — запрет изменения размера (нельзя уменьшать/увеличивать)
        ...
        logger.info("Окно отображено (полный экран, размер зафиксирован), запуск event loop...")
